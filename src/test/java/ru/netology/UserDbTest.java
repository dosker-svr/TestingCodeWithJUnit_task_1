...
public class UserDbTest {
    ...
    public void addUserInMapDbTest() {
        ...
        // Then:
        Assert.assertEquals(user, usersDb.put(user.getId(), user));
        Assert.assertSame(user, usersDb.put(user.getId(), user));
    }
    ...
    public void addIpToSetIpsForUserTest() {
        ...
        // Then:
        Assertions.assertTrue(set.add(ip));
        Assertions.assertEquals(set, ipsForUser.put(id, set));
    }
    ...
    public void getVillainTest() {
        ...
        // Then:
        Assertions.assertEquals(user.getId(), villain.getId());
    }
}

/*Вопрос - нужно ли каждый раз создавать то, что используется в каждом тестируемом методе, напрмер:
* Map<String, Set<String>> ipsForUser = new HashMap<>();
* Map<String, ru.netology.User> usersDb = new TreeMap<>();

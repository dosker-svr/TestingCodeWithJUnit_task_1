...
public class CountIpsDb {
    ...
    public void addCountIpsInMap(String string) {
        if (countOfRepeatsIps.containsKey(string)) {
            int countIp = countOfRepeatsIps.get(string);
            countOfRepeatsIps.put(string, countIp + 1);
        } else {
            countOfRepeatsIps.put(string, 1);
        }
    }
